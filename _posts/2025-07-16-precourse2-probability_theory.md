...
데이터 공간 $\mathscr{X} \times \mathscr{Y}$에 따라 확률변수가 결정되지 않고, $\mathscr{D}$에 의해 결정된다.
...
보통 컴퓨터에서 사용되는 확률변수는 이산형이지만, 기계학습의 경우 여러 종류의 분포가 있으므로 연속형 확률변수도 같이 사용하게 된다.

> 확률변수에는 이산형과 연속형만 있는 것이 아니다. 어떤 경우에는 이산형, 어떤 경우에는 연속형을 취하는 확률변수도 존재한다.
...
위의 그림은 $y$가 1이라고 주어졌을 때 조건부확률분포이다. 조건부확률분포 $P(\mathbf{x} \mid y)$는 **특정 클래스가 주어진 조건에서 데이터의 확률 분포**를 보여준다. 따라서 **입력 $\mathbf{x}$와 출력 $y$ 사이의 관계를 모델링(또는 예측)할 때 사용**된다.
...
- 이 경우 조건부확률 $P(y \mid \mathbf{x})$가 아닌 `조건부기대값` $\mathbb{E}[y \mid x]$를 추정하며, 적분으로 표현한다.
...
- 이 때, 특징패턴을 학습하기 위해 어떤 `손실함수`를 사용할지는 기계학습문제와 모델에 의해 결정된다.
