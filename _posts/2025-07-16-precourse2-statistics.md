...
확률 분포 종류는 매우 다양하며 이떄, 적절한 가정 위에서 확률 분포를 추정하는 것을 `통계적 모델링`이라고 말한다.
...
- 우선, 히스토그램을 통해 데이터의 모양을 관찰한다.
    ...
    - 데이터가 n개의 이산적인 값을 가지는 경우 &rarr; `카테고리분포`
...
이런 **`통계량`의 확률분포(즉, 표본평균과 표본분산의 확률분포)**를 `표본분포(sampling distribution)`라고 부르며, 특히 `표본평균의 표본분포`는 $N$이 커질수록 정규분포 $\mathscr{N}(\mu, \sigma^2 / N)$를 따른다.
- 이를 `중심극한정리(Central Limit Theorem)`이라 부르며, 모집단의 분포가 정규분포를 따르지 않아도 성립한다.
    - 즉, 모집단의 분포가 정규분포가 아니더라도 `표본평균의 표본분포(sampling distribution)`는 정규분포를 따를 수 있다.
- **`표본분포(sample distribution)`와 `표집분포(sampling distribution)`는 다르다**는 점에 주의하자.
    ...
    - 그러나 `표본평균의 표본분포`는 N이 커짐에 따라 정규분포를 따르게 된다.

<img src="https://blogik.netlify.app/static/d9f424fb5ca5636b3882c49d43ab9c1e/2bef9/sampling_distribution.png">


위의 사진은 베르누이확률분포(이항확률분포)를 따르는 데이터에서, 표본평균의 표본분포를 $N$이 늘어남에 따라 기록한 것이다.
...
$$
\hat{\theta}_{MLE} = \underset{\theta}{\operatorname{argmax}} \ L(\theta; x) = \underset{\mu, \sigma^2}{\operatorname{argmax}} \ P(X | \mu, \sigma^2)
$$
...
- **두 미분이 모두 0이 되는 $\mu, \sigma$**를 찾으면 가능도를 최대화하게 된다.
    ...
    - 결국 가능도를 최대화하는 추정 평균 $\hat{\mu}_{MLE}$는 데이터들의 산술평균으로, 추정 분산 $\hat{\sigma}^2_{MLE}$은 $\text{(데이터-평균)}^2$의 산술평균이 된다.
...
- 만약 해당 데이터 $x_{i, k}$가 1이 될 확률이 0이면, $p_{k}^{x_i, k}$는 $p_k^{0}$이 되어 1이 된다.
- 만약 해당 데이터 $x_{i, k}$가 1이 될 확률이 1이면, $p_k^{x_i, k}$는 $p_k$가 된다.
...
- 그러므로, $p_k = \frac{n_k}{n}$
- 즉, `카테고리분포`의 `MLE`는 각각의 class에 해당하는 count 수, 즉 **경우의수를 세어서 전체 중의 비율을 구하는 것**임을 알 수 있다.
...
$$
\hat{\theta}_{MLE} = \underset{\theta}{\operatorname{argmax}} \frac{1}{n} \sum_{i=1}^{n} \sum_{k=1}^{K} y_{i, k} \ log \ (MLP_{\theta}(x_i)_k)
$$
