...
$\mathbf{x}_i \in \mathbb{R}^k$는 $i$번째 단어에 해당하는 $k$차원의 단어 벡터라 하자. 길이가 $n$(padding 처리 필수)인 문장은 다음과 같이 표현될 수 있다.
...
본 논문에서는 모델을 테스트하기 위해 여러 벤치마크를 사용했다. 이 중 SST-2을 사용하여 모델 구현을 진행할 예정이다.

- SST-2 : 스탠포드 감정 트리뱅크(Treebank) - train/dev/test가 나뉘어 있고, 감성이 라벨링 되어 있음(좋음, 나쁨)
...
논문 내용을 바탕으로 구현한 TextCNN 모델 코드이다.
...
최종 Test Accuracy는 **84.40%**로, TextCNN 논문과 비슷한 성능을 달성하였다. 전체 구현 코드는 아래 링크에서 확인할 수 있다.

[🔗 전체 구현 코드 (GitHub)](https://github.com/Parkseojin2001/TextCNN)